...
public sealed class OAuthHandler : DelegatingHandler
{
    ...
    public OAuthHandler(IHttpClientFactory factory, IOptions<DingOptions> cfg, OAuthTokenCache cache)
    {
        ...
        _http = factory.CreateClient("oauth"); // plain client
    }
    ...
    private async Task<(string, DateTimeOffset)> FetchTokenAsync(CancellationToken ct)
    {
        ...
        return (dto.access_token, DateTimeOffset.UtcNow.AddSeconds(dto.expires_in - 60));
    }
